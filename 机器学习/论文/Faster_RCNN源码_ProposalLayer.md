...
`Proposal Layer` 在 `train` 和 `inference` 阶段都会用到。`train` 阶段：将由 `RPN` 预测的 `bbox delta` 和 `anchor` 计算出预测的 `bbox` ，以及`objectiveness score` 传给 `Proposal Target Layer` 用以判定正负标签，然后再传给 `ROIPooling Layer` 用以之后对 `ROI` 的 `class-specific` 分类；`inference` 阶段：将由 `RPN` 预测的 `bbox delta` 和 `anchor` 计算出预测的 `bbox` 直接传给 `ROIPooling Layer` 来对 `ROI` 的 `class-specific` 分类。
...
`bbox_transform_inv` 函数（将 `bbox` 和 `bbox_delta` 结合，计算出新的 `bbox`，可以理解为 `bbox_transform` 的“反函数”）：
...
完整源码：[`lib/rpn/proposal_layer.py`](https://github.com/rbgirshick/py-faster-rcnn/blob/master/lib/rpn/proposal_layer.py)
