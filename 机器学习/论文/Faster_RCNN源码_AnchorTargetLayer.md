# Faster R-CNN 源码 —— AnchorTargetLayer

`Faster R-CNN` 中关于 `AnchorTargetLayer` 的 `prototxt`:
...
layer {
  ...
  python_param {
    ...
    param_str: "{'feat_stride': 8, 'scales': [0.5, 1, 2, 4, 8, 16, 32], 'ratios': [0.333, 0.667, 1, 1.5, 3]}"
  }
}
```
...
- `bottom[0]`: `rpn_cls_score` 层传过来的 `rpn_cls_score`
- `bottom[1、2、3]`: `data` 层传过来的 `gt_boxes`, `im_info` 和 `data`，格式为 `(x1, y1, x2, y2, label)`

***1. Generate proposals from bbox deltas and shifted anchors***

首先产生了 `feature map` 的每一个位置（坐标对应于 `original image`）；然后将生成的 `anchors` 偏移到相应的上面的每一个位置，最后只保留在图片内的 `anchors`
...
生成 `bbox_targets`, `bbox_inside_weights` 和 `bbox_outside_weights`:
