...
1. 在计算 `roi_start_w`, `roi_start_h`, ... , `hstart`, `hend`, `wstart`, .... 这些变量的时候，`ROIPooling` 都是通过 round 或者 static_cast<int> 取整，即把 ROI（RPN 输出的 ROI 往往都是浮点数）映射到 feature map 上的过程中，有精确度的损失；`ROIAlign` 计算这些变量的时候都是在原本变量类型 `Dtype` （caffe 中默认为 `float32`）进行计算
...
因为在计算 `hstart`, `hend`, `wstart`, `wend` 的过程中，`ROIPooling` 采取的办法是取整，所以自然可以映射到 feature map 上，故 `ROIPooling` 直接在 `ROI` 的每一个 `bin` 内布进行 `Max Pooling`；而 `ROIAlign` 因为计算的 `hstart`, `hend`, `wstart`, `wend` 是浮点数，所以 `h`, `w` 不能映射到 feature map 上，所以采取的策略是：对于每一个浮点数 `h`, `w`，分别进行向上和向下取整，然后根据这四个位置（也是最邻近的四个位置）进行双线性插值，得出 `h`, `w` 对应的值，然后再在 `ROI` 的每一个 `bin` 内布进行 `Max Pooling`
